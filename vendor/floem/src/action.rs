...
/// Set the surrounding text, UTF-16 selection range, and optional marked range for platform IME queries.
pub fn set_ime_text_context(
    text: String,
    selection: (usize, usize),
    marked_range: Option<(usize, usize)>,
) {
    add_update_message(UpdateMessage::SetImeTextContext {
        text,
        selection,
        marked_range,
    });
