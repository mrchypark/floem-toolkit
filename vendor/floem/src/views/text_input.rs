use crate::action::{exec_after, set_ime_allowed, set_ime_cursor_area, set_ime_text_context};
...
use std::{any::Any, ffi::OsString, fs::OpenOptions, io::Write, ops::Range};
...
struct TextInputPreedit {
    text: String,
    cursor: Option<(usize, usize)>,
}

impl TextInputPreedit {
...
fn should_clear_ime_preedit(has_local_preedit: bool, commit_pending: bool) -> bool {
...
fn ime_debug_enabled() -> bool {
    ime_debug_enabled_from(|key| std::env::var_os(key))
}

fn ime_debug_emit(line: impl AsRef<str>) {
    let line = line.as_ref();
    eprintln!("{line}");

    if let Some(path) = std::env::var_os("FLOEM_SHOWCASE_IME_STATE_LOG_FILE") {
...
fn byte_to_utf16_index(text: &str, byte_idx: usize) -> usize {
...
pub struct TextInput {
    ...
    is_focused: bool,
    ime_preedit: Option<TextInputPreedit>,
...
pub fn text_input(buffer: RwSignal<String>) -> TextInput {
    ...
    .on_event_stop(EventListener::FocusGained, move |_| {
        ...
        if ime_debug_enabled() {
            eprintln!(
                "[floem-text-input] label={:?} stage=focus_gained",
                id,
            );
        }
    ...
    .on_event_stop(EventListener::FocusLost, move |_| {
        ...
        if ime_debug_enabled() {
            eprintln!(
                "[floem-text-input] label={:?} stage=focus_lost",
                id,
            );
        }
...
impl TextInput {
    fn ime_debug_label(&self) -> String {
    ...
    fn ime_debug_log(&self, stage: &str) {
        if !ime_debug_enabled() {
            return;
        }
        ime_debug_emit(format!(
            "[floem-text-input] label={} stage={} focused={} cursor={} buffer={:?} preedit={:?} commit_pending={}",
            self.ime_debug_label(),
            stage,
            self.is_focused,
            self.cursor_glyph_idx,
            self.buffer.get_untracked(),
            self.ime_preedit.as_ref().map(|preedit| preedit.text.clone()),
            self.ime_commit_pending,
        ));
    }

    fn preedit_insert_idx(&self) -> usize {
        clamp_text_boundary(&self.buffer.get_untracked(), self.cursor_glyph_idx)
    }

    fn displayed_cursor_glyph_idx(&self) -> usize {
        let insert_idx = self.preedit_insert_idx();
        insert_idx
            + self
                .ime_preedit
                .as_ref()
                .map(|preedit| preedit.display_cursor_offset())
                .unwrap_or_default()
    }
    ...
    fn displayed_text(&self) -> String {
        display_text_with_preedit(
            &self.buffer.get_untracked(),
            self.preedit_insert_idx(),
            self.ime_preedit.as_ref(),
        )
    }

    fn buffer_idx_from_display_idx(&self, display_idx: usize) -> usize {
        let buffer = self.buffer.get_untracked();
        let display_idx = display_idx.min(self.displayed_text().len());
        if let Some(preedit) = &self.ime_preedit {
            let insert_idx = self.preedit_insert_idx();
            let preedit_end = insert_idx + preedit.text.len();
            if display_idx <= insert_idx {
                clamp_text_boundary(&buffer, display_idx)
            } else if display_idx <= preedit_end {
                insert_idx
            } else {
                clamp_text_boundary(&buffer, display_idx - preedit.text.len())
            }
        } else {
            clamp_text_boundary(&buffer, display_idx)
        }
    ...
    fn clear_preedit(&mut self) {
        ...
        self.ime_debug_log("clear_preedit");
    }

    fn set_preedit(&mut self, text: String, cursor: Option<(usize, usize)>) {
        self.ime_preedit = (!text.is_empty()).then_some(TextInputPreedit { text, cursor });
        self.ime_debug_log("set_preedit");
    }

    fn commit_ime_text(&mut self, text: &str) {
        self.clear_preedit();
        if text.is_empty() {
            self.ime_debug_log("commit_ime_text:empty");
            return;
        }

        if let Some(selection) = self.selection.clone() {
            self.buffer
                .update(|buf| replace_range(buf, selection.clone(), Some(text)));
            self.cursor_glyph_idx = selection.start + text.len();
            self.selection = None;
            self.ime_debug_log("commit_ime_text:selection");
            return;
        }
    ...
    fn clip_text(&mut self, node_layout: &Layout) {
        let rendered_text = self.displayed_text();
        let display_cursor_glyph_idx = self.displayed_cursor_glyph_idx();
        let virt_text = self.text_buf.as_mut().unwrap();
        let node_width = node_layout.size.width as f64;
        let cursor_text_loc = Cursor::new(0, display_cursor_glyph_idx);
        ...
        let clip_start =
            clamp_text_boundary(&rendered_text, virt_text.hit_point(Point::new(clip_start_x, 0.0)).index);
        let clip_end = clamp_text_boundary(
            &rendered_text,
            virt_text
        ...
        let new_text = rendered_text[clip_start..clip_end].to_string();
    ...
    fn get_box_position(&self, pos_x: f64, pos_y: f64) -> usize {
        ...
        let display_idx = self
            ...
            .index;
        self.buffer_idx_from_display_idx(display_idx)
    }
    ...
    fn update_text_layout(&mut self) {
        ...
        let attrs_list = self.get_text_attrs();
        let rendered_text = self.displayed_text();
        text_layout.set_text(&rendered_text, attrs_list.clone());
    ...
    fn handle_key_down(&mut self, cx: &mut EventCx, event: &KeyEvent) -> bool {
        if ime_debug_enabled() {
            eprintln!(
                "[floem-text-input] label={} stage=keydown:start focused={} cursor={} logical={:?} text={:?} preedit={:?} buffer={:?} commit_pending={}",
                self.ime_debug_label(),
                self.is_focused,
                self.cursor_glyph_idx,
        ...
        if handled {
            self.ime_debug_log("keydown:handled");
            return true;
        }
        ...
        match event.key.logical_key {
            Key::Character(ref ch) => {
                ...
                let inserted = self.insert_text(ch);
                if inserted {
                    self.ime_debug_log("keydown:character_inserted");
                }
                inserted
            }
            _ => false,
    ...
    fn event_before_children(&mut self, cx: &mut EventCx, event: &Event) -> EventPropagation {
        ...
        let is_handled = match &event {
            ...
            Event::PointerDown(
                ...
                self.ime_debug_log("pointer_down:before");
                ...
                self.ime_debug_log("pointer_down:after");
                true
            }
            ...
            Event::ImePreedit { text, cursor } => {
                if ime_debug_enabled() {
                    eprintln!(
                        "[floem-text-input] label={} stage=ime_preedit:start text={:?} cursor={:?} focused={} buffer={:?} preedit={:?} commit_pending={}",
                        self.ime_debug_label(),
                        text,
                        cursor,
                        self.is_focused,
            ...
            Event::ImeCommit(text) => {
                if ime_debug_enabled() {
                    eprintln!(
                        "[floem-text-input] label={} stage=ime_commit:start text={:?} focused={} buffer={:?} preedit={:?} commit_pending={}",
                        self.ime_debug_label(),
                        text,
                        self.is_focused,
                        self.buffer.get_untracked(),
            ...
            Event::ImeDisabled => {
                self.ime_debug_log("ime_disabled:start");
                let should_handle = self.ime_preedit.is_some() || self.ime_commit_pending;
                if should_handle {
                    self.clear_preedit();
                    self.ime_commit_pending = false;
                    self.ime_debug_log("ime_disabled:handled");
                }
                should_handle
            }
    ...
    fn compute_layout(&mut self, _cx: &mut crate::context::ComputeLayoutCx) -> Option<Rect> {
        ...
        if buf_width > node_width {
            ...
            let hit_pos = self
                ...
                .as_ref()
                .unwrap()
                .hit_position(self.displayed_cursor_glyph_idx());
...
fn clamp_text_boundary(text: &str, idx: usize) -> usize {
...
fn display_text_with_preedit(
    ...
    let mut rendered = String::with_capacity(buffer.len() + preedit.text.len());
    rendered.push_str(&buffer[..insert_idx]);
    rendered.push_str(&preedit.text);
    rendered.push_str(&buffer[insert_idx..]);
    rendered
}
...
mod tests {
    ...
    #[test]
    fn preedit_display_inserts_text_at_cursor() {
        let rendered = display_text_with_preedit(
            "ab",
            1,
        ...
        assert_eq!(rendered, "a한b");
    }

    #[test]
    fn preedit_display_uses_end_when_cursor_past_buffer() {
        let rendered = display_text_with_preedit(
            "ab",
            9,
            Some(&TextInputPreedit {
        ...
        assert_eq!(rendered, "ab글");
    }

    #[test]
    fn focused_input_accepts_preedit() {
        assert!(should_accept_ime_preedit(true));
        assert!(!should_accept_ime_preedit(false));
    }

    #[test]
    fn local_preedit_can_be_cleared_without_focus() {
        assert!(should_clear_ime_preedit(true, false));
        assert!(should_clear_ime_preedit(false, true));
        assert!(!should_clear_ime_preedit(false, false));
    }
    ...
    #[test]
    fn ime_debug_enabled_accepts_new_primary_flag() {
        let enabled = ime_debug_enabled_from(|key| {
            (key == "FLOEM_IME_DEBUG").then(|| OsString::from("1"))
        });
    ...
    #[test]
    fn ime_debug_enabled_accepts_legacy_flag() {
        let enabled = ime_debug_enabled_from(|key| {
